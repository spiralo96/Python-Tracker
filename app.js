...
function refreshSidebarProgress() {
  ...
  const doneCount = Object.entries(state.progress).filter(
    ([k, s]) => k.startsWith("topic-") && DONE_STATES.has(s)
  ).length;
...
function subKey(topicNum, idx) {
  return `sub-${topicNum}-${idx}`;
}

function isSubDone(key) {
  return state.progress[key] === "comfortable";
}

function allSubitemsDone(topic) {
  if (!topic.items.length) return false;
  return topic.items.every((_, idx) => isSubDone(subKey(topic.num, idx)));
}
...
function buildTopicRow(topic) {
  ...
  row.innerHTML = `
    <div class="topic-main">
      <button class="topic-check ${DONE_STATES.has(status) ? "checked" : ""}" title="Mark comfortable / not started" aria-label="Toggle done">
        <svg viewBox="0 0 24 24" fill="none" stroke="#1b1917" stroke-width="3"><path d="M4 12l5 5L20 7"/></svg>
      </button>
    ...
    <div class="topic-sub">
      <ul>
        ${topic.items.map((it, idx) => `
          <li>
            <button class="sub-check ${isSubDone(subKey(topic.num, idx)) ? "checked" : ""}" data-idx="${idx}" aria-label="Toggle subtopic done">
              <svg viewBox="0 0 24 24" fill="none" stroke="#1b1917" stroke-width="3"><path d="M4 12l5 5L20 7"/></svg>
            </button>
            <span class="sub-label">${formatItem(it)}</span>
          </li>
        `).join("")}
      </ul>
    </div>
  ...
  row.querySelector(".topic-check").addEventListener("click", () => {
    ...
    // cascade to every subitem so the parent/child checkboxes stay in sync
    const targetSubStatus = newStatus === "comfortable" ? "comfortable" : "not_started";
    topic.items.forEach((_, idx) => {
      const sKey = subKey(topic.num, idx);
      state.progress[sKey] = targetSubStatus;
      saveProgress(sKey, targetSubStatus);
    });
    row.querySelectorAll(".sub-check").forEach((btn) => {
      btn.classList.toggle("checked", targetSubStatus === "comfortable");
    });
  });
  ...
  row.querySelectorAll(".sub-check").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const sKey = subKey(topic.num, idx);
      const nowDone = !isSubDone(sKey);
      const newSubStatus = nowDone ? "comfortable" : "not_started";
      state.progress[sKey] = newSubStatus;
      btn.classList.toggle("checked", nowDone);
      saveProgress(sKey, newSubStatus);

      // if every subitem is now done, auto-complete the parent topic
      // (never downgrades an already-higher status like "Can teach")
      if (allSubitemsDone(topic)) {
        const current = statusOfTopic(key);
        if (!DONE_STATES.has(current)) {
          setTopicStatus(key, "comfortable", row);
        }
      }
    });
  });
...
function buildReferenceSections() {
  ...
  // Parallel tracks — tabbed, with their own (non-counted) checkboxes
  const ptSection = document.createElement("section");
  ptSection.className = "ref-section";
  ptSection.id = "parallel-tracks";
  ptSection.innerHTML = `
    <div class="level-header"><h2 class="level-title">Parallel tracks</h2></div>
    <div class="track-note">Optional side-tracks — checking these off is just for you; they don't factor into the 305-topic progress above.</div>
    <div class="track-tabs" id="trackTabs"></div>
    <div class="track-panels" id="trackPanels"></div>
  `;
  container.appendChild(ptSection);

  const tabsEl = ptSection.querySelector("#trackTabs");
  const panelsEl = ptSection.querySelector("#trackPanels");

  ROADMAP_DATA.parallelTracks.forEach((track, trackIdx) => {
    const tab = document.createElement("button");
    tab.className = "track-tab" + (trackIdx === 0 ? " active" : "");
    tab.textContent = track.name;
    tab.dataset.idx = trackIdx;
    tab.addEventListener("click", () => {
      tabsEl.querySelectorAll(".track-tab").forEach((t) => t.classList.remove("active"));
      panelsEl.querySelectorAll(".track-panel").forEach((p) => p.classList.remove("active"));
      tab.classList.add("active");
      panelsEl.querySelector(`.track-panel[data-idx="${trackIdx}"]`).classList.add("active");
    });
    tabsEl.appendChild(tab);

    const panel = document.createElement("div");
    panel.className = "track-panel" + (trackIdx === 0 ? " active" : "");
    panel.dataset.idx = trackIdx;

    track.subsections.forEach((sub, subIdx) => {
      if (sub.name !== "_flat") {
        const subTitle = document.createElement("div");
        subTitle.className = "track-sub-title";
        subTitle.textContent = sub.name;
        panel.appendChild(subTitle);
      }
      const ul = document.createElement("ul");
      ul.className = "track-check-list";
      sub.items.forEach((item, itemIdx) => {
        const tKey = `track-${trackIdx}-${subIdx}-${itemIdx}`;
        const li = document.createElement("li");
        li.innerHTML = `
          <button class="sub-check ${state.progress[tKey] === "comfortable" ? "checked" : ""}" aria-label="Toggle item done">
            <svg viewBox="0 0 24 24" fill="none" stroke="#1b1917" stroke-width="3"><path d="M4 12l5 5L20 7"/></svg>
          </button>
          <span class="sub-label">${escapeHtml(item)}</span>
        `;
        li.querySelector(".sub-check").addEventListener("click", (e) => {
          const nowDone = !(state.progress[tKey] === "comfortable");
          const newStatus = nowDone ? "comfortable" : "not_started";
          state.progress[tKey] = newStatus;
          e.currentTarget.classList.toggle("checked", nowDone);
          saveProgress(tKey, newStatus);
        });
        ul.appendChild(li);
      });
