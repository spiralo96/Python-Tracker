...
## How subtopic checkboxes work

Expanding a topic (click its title) shows a checkbox per subtopic. Check them
individually to track fine-grained progress:

- Checking **every** subtopic under a topic auto-marks that topic
  "Comfortable" — no need to also touch the dropdown.
- Unchecking a single subtopic afterward does **not** downgrade the topic —
  that stays a deliberate action via the dropdown or the main checkbox, so a
  slip of the mouse doesn't wipe out a status you set on purpose.
- Clicking the main topic checkbox also cascades: checking it checks every
  subtopic, unchecking it unchecks every subtopic.

Subtopic state is saved the same way as everything else (per code, in
Supabase) but isn't counted in the 305-topic overall percentage — that number
only reflects the main topics. No schema changes are needed for this — it
reuses the same `not_started` / `comfortable` values your `schema.sql` already
allows, just under different key prefixes (`sub-...`, `track-...` instead of
`topic-...`).

## Parallel tracks

The Linux / C / C++ / Luau tracks live in their own tabbed section further
down the page, with their own checkboxes. These are optional side-reading,
so — like subtopics — they save to your profile but are **excluded from the
overall progress percentage**, which only tracks the core 305-topic
curriculum.
